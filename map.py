...
class Map():
    ...
    def __init__(self, size = 5, pos = Coord(1,1), hero = "@"):
        self.size = size
        self._mat = [[Map.ground for i in range(self.size)] for j in range(self.size)] # used to initialize the map with ground cells
        self._elem = {hero : pos} # used to store the elements of the map and their position
        for el in self._elem: # used to place the elements on the map
            posX = self._elem[el].x
            posY = self._elem[el].y
            self._mat[posY][posX] = el
    ...
    def __contains__(self, item):
        '''Returns True if item's coordinates are in the map, or if item is in the map. Returns False otherwise.'''
    ...
    def get(self, coords):
        '''Returns the element at the coordinates coords. Returns None if coords is not in the map.'''
        if coords in self:
    ...
    def rm(self, coords):
        ...
        if coords in self:
            ...
            for el in self._elem:
                if self._elem[el] == coords:
                    del self._elem[el]
                    break
    ...
    def move(self, element, way):
        ...
        if element in self._elem:
            if self.pos(element)+way in self and self.get(self.pos(element)+way) == Map.ground: # if the cell is empty
                posIni= self.pos(element)
                self.rm(posIni)
                self.put(posIni+way, element)
